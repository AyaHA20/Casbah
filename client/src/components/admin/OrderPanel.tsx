import { useState } from 'react'
import type { AdminOrderDetail, OrderStatus } from '../../lib/api'
import { fmtDA, fmtPhone } from '../../lib/format'
import { DELIVERY_LABEL, STATUS_LABEL, telHref } from '../../lib/status'

const TIME_FMT = new Intl.DateTimeFormat('fr-DZ', { hour: '2-digit', minute: '2-digit' })

type Props = {
  order: AdminOrderDetail | null
  loading: boolean
  onStatusChange: (status: OrderStatus) => Promise<void>
}

export function OrderPanel({ order, loading, onStatusChange }: Props) {
  const [pending, setPending] = useState(false)
  const [error, setError] = useState<string | null>(null)

  if (loading) {
    return <p className="p-8 text-center text-ink-soft">Chargement…</p>
  }

  if (!order) {
    return (
      <p className="p-8 text-center text-meta text-ink-soft">
        Sélectionnez une commande pour voir le détail.
      </p>
    )
  }

  async function change(next: string) {
    if (!next) return
    setPending(true)
    setError(null)
    try {
      await onStatusChange(next as OrderStatus)
    } catch (e) {
      setError(e instanceof Error ? e.message : 'Changement impossible.')
    } finally {
      setPending(false)
    }
  }

  return (
    <div className="flex flex-col gap-[22px] p-gutter lg:p-8">
      <div className="flex items-baseline justify-between">
        <h2 className="text-[26px] lg:text-[34px]">{order.orderNumber}</h2>
        <span className="text-meta text-ink-soft">
          Reçue {TIME_FMT.format(new Date(order.createdAt))}
        </span>
      </div>

      <dl className="flex flex-col gap-[10px] border-t border-line pt-[18px] text-sm lg:text-[15px]">
        <Row label="Client" value={order.customerName} />
        <div className="flex justify-between">
          <dt className="text-ink-soft">Téléphone</dt>
          <dd>
            {/* tel: so the shop can tap to call straight from the panel */}
            <a href={telHref(order.phone)} className="font-semibold text-green hover:text-rust">
              {fmtPhone(order.phone)}
            </a>
          </dd>
        </div>
        <Row label="Wilaya" value={`${order.wilaya.code} — ${order.wilaya.nameFr}`} />
        <Row label="Commune" value={order.commune.name} />
        <Row
          label="Mode"
          value={order.deliveryType === 'DESK' ? 'Stop desk' : 'À domicile'}
        />
        <div className="flex justify-between gap-6">
          <dt className="flex-none text-ink-soft">Adresse</dt>
          <dd className="text-right">{order.address}</dd>
        </div>
        {order.notes && <Row label="Note" value={order.notes} />}
      </dl>

      <div className="flex flex-col gap-[10px] border-t border-line pt-[18px] text-sm lg:text-[15px]">
        {order.items.map((item) => (
          <div key={item.id} className="flex justify-between gap-4">
            <span>
              {item.productName} — {item.variantSize} / {item.variantColor}
              {item.quantity > 1 && ` ×${item.quantity}`}
            </span>
            <span className="whitespace-nowrap">{fmtDA(item.unitPrice * item.quantity)}</span>
          </div>
        ))}
        <div className="flex justify-between">
          <span className="text-ink-soft">
            Livraison {DELIVERY_LABEL[order.deliveryType]}
          </span>
          <span>{fmtDA(order.shipping)}</span>
        </div>
        <div className="flex items-baseline justify-between border-t border-ink pt-3">
          <span className="font-semibold">À encaisser</span>
          <span className="font-display text-[24px] font-bold lg:text-[30px]">
            {fmtDA(order.total)}
          </span>
        </div>
      </div>

      <div className="flex flex-col gap-2">
        <label htmlFor="statut" className="text-meta text-ink-soft">
          Statut de la commande
        </label>
        <select
          id="statut"
          value=""
          disabled={pending || order.allowedTransitions.length === 0}
          onChange={(e) => void change(e.target.value)}
          className="w-full appearance-none rounded-[12px] border border-green bg-cream p-[13px] text-sm font-semibold text-ink disabled:border-line disabled:bg-field disabled:text-ink-soft lg:p-[14px] lg:text-[15px]"
        >
          <option value="">
            {STATUS_LABEL[order.status]}
            {order.allowedTransitions.length === 0 ? ' — état final' : ' — changer pour…'}
          </option>
          {/* Only legal moves appear. The server enforces the same table. */}
          {order.allowedTransitions.map((s) => (
            <option key={s} value={s}>
              {STATUS_LABEL[s]}
            </option>
          ))}
        </select>

        {order.stockRestored && (
          <span className="text-meta text-ink-soft">
            Stock déjà remis en rayon pour cette commande.
          </span>
        )}
        {error && (
          <span className="rounded-md border border-rust/40 bg-rust/5 p-2.5 text-meta text-rust">
            {error}
          </span>
        )}
      </div>

      <div className="flex items-center gap-3">
        <a
          href={telHref(order.phone)}
          className="flex-1 rounded-pill border border-green bg-green py-[14px] text-center text-sm font-semibold text-cream lg:py-[15px] lg:text-[15px]"
        >
          Appeler le client
        </a>
        <a
          href={telHref(order.phone)}
          aria-hidden
          tabIndex={-1}
          className="grid h-12 w-12 flex-none place-items-center rounded-pill border border-green text-lg text-green lg:h-[52px] lg:w-[52px]"
        >
          ↗
        </a>
      </div>

      <p className="text-meta leading-[1.6] text-ink-soft">
        Confirmez par téléphone avant d'imprimer le bordereau. Toute commande non confirmée après
        48 h passe en attente.
      </p>
    </div>
  )
}

function Row({ label, value }: { label: string; value: string }) {
  return (
    <div className="flex justify-between gap-4">
      <dt className="text-ink-soft">{label}</dt>
      <dd className="text-right">{value}</dd>
    </div>
  )
}
